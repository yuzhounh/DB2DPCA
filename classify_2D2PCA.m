function classify_2D2PCA(database,classifier,nPC)
% classify by (2D)2PCA
...
for iRep=1:nRep
    ...
    for iCV=1:nCV
        ...
        for iK=1:nK
            ...
            % projection
            x_train_proj=zeros(k,k,nTrain);
            for iTrain=1:nTrain
                x_train_proj(:,:,iTrain)=v'*x_train(:,:,iTrain)*u;
            end
            
            x_test_proj=zeros(k,k,nTest);
            for iTest=1:nTest
                x_test_proj(:,:,iTest)=v'*x_test(:,:,iTest)*u;
            end
            
            % reshape each sample into a vector
            x_train_proj=reshape(x_train_proj,k*k,nTrain);
            x_test_proj=reshape(x_test_proj,k*k,nTest);
...
save(sprintf('%s/Acc_2D2PCA_%s.mat',db,classifier),'accuracy','time');